...
feature "Beszállított anyag szerkesztése" do
  ...
  scenario "címe 'AK-Anyag beszállítás'" do
    expect(page).to have_title('AK-Anyag beszállítás')
    expect(page).not_to have_title('AK - AK-Anyag beszállítá')
  end
  ...
  scenario "helyesen tartalmazza az anyag beszállítása oldal elemeit" do
    ...
    page.should have_selector('h3', text: 'Anyag beszállítás')
    expect(page).to have_text('A beszállított anyag neve')
    expect(page).to have_field("current_item_id", text: '38-as N+F tégla')
    expect(page).to have_text('Mennyiség az anyagból')
    expect(page).to have_field("current_quantity")
    expect(page).to have_text('A bizonylat neve és száma')
